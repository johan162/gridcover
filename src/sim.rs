...
pub fn simulation_loop(model: &mut SimModel, rng: &mut impl Rng) {
    ...
    let mut cutter_center_pos = Vector {
    ...
    while (model.stop_bounces == 0
        || (model.stop_bounces > 0 && model.bounce_count < model.stop_bounces))
        && (model.stop_time == 0.0
        ...
        && (model.stop_distance == 0.0
            || model.stop_distance > 0.0
                && model.distance_covered < model.stop_distance)
        && model.sim_time_elapsed < FAILSAFE_TIME_LIMIT
    {
        ...
        cutter_center_pos += current_dir * model.step_size;

        // Find and mark all grid cells that are fully covered by the circle at the current position
        mark_covered_cells(&cutter_center_pos, model);

        // Check for collisions with boundaries
        let collision_detected =
            check_collision(&cutter_center_pos, &model.bb, &mut current_dir);

        if collision_detected {
            model.bounce_count += 1;
            // Get the position un-collided
            cutter_center_pos = Vector {
                x: model.bb.limit_x(cutter_center_pos.x),
                y: model.bb.limit_y(cutter_center_pos.y),
            };
        }

        (current_dir.x, current_dir.y) = cutter_strategy(
            &current_dir,
            &cutter_center_pos,
            collision_detected,
            model,
            ...
            if model.show_progress {
                if model.battery_run_time > 0.0 {
                    model.battery_charge_left = 100.0
                        - (sim_time_elapsed_since_last_charge
                            / (model.battery_run_time * 60.0))
                            * 100.0;
                    print!(
