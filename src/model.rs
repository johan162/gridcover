        ...
        let json = json!({
            "Result": {
                ...
                "Time": {
                    ...
                    "CPU time (milliseconds)": self.cpu_time.num_milliseconds(),
                    "Cutting time": format!("{:02}:{:02}:{:02}",
                        hours, minutes, seconds),
                    "Cutting time (seconds)": self.sim_time_elapsed as u64,
                    "Min.Cov.Time": format!("{:02}:{:02}:{:02}",
                        t_hours, t_minutes, t_seconds),
                    "Min.Cov.Time (seconds)": t_hours * 3600 + t_minutes * 60 + t_seconds,
